...
export const COACH_CONTEXT_V1_SOURCE_MAP: readonly CoachContextV1SourceMapEntry[] = Object.freeze([
  ...
  },
  {
    ...
    rule: "Request-scoped metadata only; no conversation-history authority.",
  },
  {
    fieldPattern: "userId",
  ...
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "User-owned active profile only; the existing reusable planner reader selects too few identity fields.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Server-side user-profile projection is required before 6B.2; client state is not a Coach authority boundary.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "The date and all query windows must be resolved server-side with the same supported timezone.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Compact identity projection; no raw auth, profile, or edition row.",
  },
  {
  ...
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Deterministic Today projection over normalized task state; CoachContext copies the projection and does no planning arithmetic.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Actual, planned actual, planned credit, Extra Study, and unknown intent remain separate and use timezone-bounded queries.",
  },
  {
  ...
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Coverage is sufficient only when the requested window has overlap-safe, reconciled PLN-002 intent evidence.",
  },
  {
  ...
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "User-profile subject selection joined to canonical subject identity.",
  },
  {
    ...
    existingReaderOrContract: ["packages/domain/src/planning-v2/db-snapshot-contract.ts::normalizePlanningSnapshotDbBundleV1"],
    readiness: "adapter_extraction_required",
    rule: "Deterministic grouping of normalized canonical tasks by subjectId.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Deterministic grouping by explicit subject identity; title inference is forbidden.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Group canonical material/workload outputs by supplied subjectId; never infer from title or legacy top-three summaries.",
  },
  {
  ...
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Extract a read-only capacity projection from the current adaptive monolith; do not import legacy Coach target-capacity or mutate capacity.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Bounded window and compact event projection; no raw task rows.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Bounded completed sessions with explicit accounting intent and recording channel; no notes or raw rows.",
  },
  {
    ...
    readiness: "adapter_extraction_required",
    rule: "Carry exact typed lifecycle/identity/minute/date facts only; no transition is inferred from task movement.",
  },
  {
    ...
    existingReaderOrContract: [
      ...
      "app-api loadPlannerV2Proposal: user-scoped confirmed_action_proposals action_kind=planner_v2_week",
    ],
    readiness: "adapter_extraction_required",
    rule: "Persisted lifecycle state and exact proposal identity only; no conversational confirmation or Apply authority.",
  },
  {
    fieldPattern: "planner.value.explanationFacts",
    truthSources: ["planner_v2_lifecycle"],
    existingReaderOrContract: ["packages/domain/src/planning-v2/proposal-lifecycle.ts::buildPlannerV2Preview"],
    readiness: "reusable_now",
    rule: "Copy structured canonical preview facts; AI may later phrase but not alter them.",
  },
  {
