...
export async function loadAdaptiveBase(client:Client,userId:string,profile:any,plan:any){
 const [availability,calendar,exceptionRows,tasks,progress,revisions,reschedules,sessions]=await Promise.all([
  ...
  client.from("study_sessions").select("duration_minutes,ended_at").eq("user_id",userId).eq("exam_profile_id",profile.id).eq("status","completed").gte("ended_at",`${plan.week_start_date}T00:00:00Z`),
 ...
 return{availability:availability.data??[],calendar:calendar.data??[],exceptions:exceptionRows.data??[],tasks:tasks.data??[],adaptiveTasks,adaptiveRevisions,allAdaptiveRevisions,dayCapacities,actualMinutes:(sessions.data??[]).reduce((s:number,row:any)=>s+(row.duration_minutes??0),0)};
}
...
export async function recalculateCurrentPlan(client:Client,userId:string,profile:any,plan:any,trigger:any,serviceRole=false){
 ...
 const output=replanWeeklyPlanV1({profileId:profile.id,planId:plan.id,weekStart:plan.week_start_date,weekEnd:plan.week_end_date,currentDate:today,planningBudgetMinutes:Math.floor(Object.values(base.dayCapacities).reduce((a:number,b:any)=>a+b,0)*.85),dailyCapacities:base.dayCapacities,tasks:base.adaptiveTasks,revisions:base.adaptiveRevisions,trigger});
 ...
 const snapshotKey=`${backlog.openTaskCount}:${backlog.estimatedRemainingMinutes}:${backlog.remainingCapacityMinutes}:${backlog.severity}|${risks.map(row=>row.riskType).sort().join(",")}`;const payload={weeklyPlanId:plan.id,...output,dedupeKey:`${output.dedupeKey}|snapshot:${snapshotKey}`,explanation:output.changedTaskCount===0&&risks.length?"Kapasite ve risk durumu güncel koşullara göre yenilendi.":output.explanation,planningBudgetMinutes:Math.floor(output.availableMinutes*.85),backlog:{...backlog,capacityRatio:Number.isFinite(backlog.capacityRatio)?backlog.capacityRatio:999},risks};
