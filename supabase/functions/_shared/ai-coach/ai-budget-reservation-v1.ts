...
export async function reserveAiProviderBudgetV1(input: {
  ...
  if (authorization.version !== AI_PROVIDER_COST_AUTHORIZATION_V1_VERSION || authorization.runtimeEnvironment !== input.route.runtimeEnvironment || authorization.provider !== input.route.provider || authorization.modelId !== input.route.modelId || authorization.modelTier !== input.route.tier || authorization.routeCatalogVersion !== input.route.catalogVersion || authorization.pricingVersion !== input.route.pricingVersion || !authorization.fxPolicyVersion.trim() || !authorization.fxSnapshotVersion.trim() || !authorization.billingBoundVersion.trim() || !authorization.providerRequestFingerprint.trim() || !authorization.inputCountVersion.trim() || !Number.isInteger(authorization.inputTokenUpperBound) || authorization.inputTokenUpperBound <= 0 || !Number.isInteger(authorization.outputTokenUpperBound) || authorization.outputTokenUpperBound !== input.route.maxOutputTokens || !Number.isFinite(authorization.tryMaximum) || authorization.tryMaximum <= 0 || authorization.tryMaximum > 300) throw new Error("AI_BUDGET_COST_AUTHORIZATION_INVALID");
