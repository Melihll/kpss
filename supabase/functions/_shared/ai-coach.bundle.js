...
// packages/domain/src/ai-coach/conversation-intelligence-v1.ts
var COACH_CONVERSATION_CONTEXT_V1_VERSION = "coach-conversation-context-v1";
var COACH_CONVERSATION_MAX_EXCHANGES_V1 = 3;
var COACH_CONVERSATION_MAX_TURNS_V1 = 6;
var COACH_CONVERSATION_TURN_MAX_LENGTH_V1 = 1200;
var COACH_CONVERSATION_TOTAL_MAX_LENGTH_V1 = 7200;
var COACH_CONVERSATION_CURRENT_MESSAGE_MAX_LENGTH_V1 = 2e3;
...
function normalizeText(value, maxLength, errorCode) {
  if (typeof value !== "string") {
    throw new Error(errorCode);
  }
  const normalized = value.normalize("NFC").trim();
  if (!normalized || [...normalized].length > maxLength || /[\u0000-\u001f\u007f]/u.test(
    normalized
  )) {
    throw new Error(errorCode);
  }
...
function searchableMessage(value) {
  return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replaceAll("\u0131", "i").toLocaleLowerCase("tr-TR").replace(/[^\p{L}\p{N}\s]/gu, " ").replace(/\s+/g, " ").trim();
}
function isCoachConversationFollowUpV1(rawMessage) {
  const message = searchableMessage(rawMessage);
  return /^(peki|peki neden|neden|neden boyle|nasil yani|ne demek|devam et)$/.test(message) || message === "biraz daha aciklar misin" || message === "biraz daha acikla" || message === "bunu biraz daha acikla" || message === "onu biraz daha acikla";
}
function parseCoachConversationInputV1(value) {
  if (value === void 0 || value === null) {
    return null;
  }
  if (!isRecord2(value) || !exactKeys(
    value,
    [
      "version",
      "recentTurns"
    ]
  ) || value.version !== COACH_CONVERSATION_CONTEXT_V1_VERSION || !Array.isArray(
    value.recentTurns
  )) {
    throw new Error(
  ...
  if (value.recentTurns.length > COACH_CONVERSATION_MAX_TURNS_V1 || value.recentTurns.length % 2 !== 0) {
    throw new Error(
      "COACH_CONVERSATION_TURN_COUNT_INVALID"
    );
  }
  let totalLength = 0;
  const recentTurns = value.recentTurns.map(
    (candidate, index) => {
      if (!isRecord2(candidate) || !exactKeys(
        candidate,
        [
          "role",
          "text"
  ...
  if (totalLength > COACH_CONVERSATION_TOTAL_MAX_LENGTH_V1) {
    throw new Error(
      "COACH_CONVERSATION_TOTAL_LENGTH_INVALID"
    );
  }
  return deepFreeze11({
    version: COACH_CONVERSATION_CONTEXT_V1_VERSION,
    recentTurns
  });
}
...
async function sha256(value) {
  ...
  const hex = [...new Uint8Array(digest)].map(
    (byte) => byte.toString(16).padStart(2, "0")
  ).join("");
  return `sha256:${hex}`;
}
function subjectMatches(context, rawText) {
  const message = searchableMessage(
    rawText
  );
  return context.subjects.map(
    (subject) => {
      const normalizedName = searchableMessage(
        subject.subjectName
      );
      return {
        subjectId: subject.subjectId,
    ...
    (candidate) => candidate.normalizedName.length > 0 && candidate.index >= 0
  );
}
function explicitSubjectMatch(context, rawText) {
  ...
  if (unique.length === 1) {
    ...
    return {
      status: "resolved",
      subjectId: onlySubject.subjectId
    };
  }
  const normalized = searchableMessage(
    rawText
  );
  const rawLower = rawText.normalize("NFC").toLocaleLowerCase("tr-TR");
  const rawCorrectionMarker = rawLower.lastIndexOf(
    "de?il"
  );
  const normalizedCorrectionMarker = normalized.lastIndexOf(
    "degil"
  );
  if (rawCorrectionMarker >= 0 || normalizedCorrectionMarker >= 0) {
    const correctedPart = rawCorrectionMarker >= 0 ? searchableMessage(
      rawText.slice(
        rawCorrectionMarker + "de?il".length
      )
    ) : normalized.slice(
      normalizedCorrectionMarker + "degil".length
    ).trim();
    const corrected = unique.filter(
      (candidate) => correctedPart.includes(
        candidate.normalizedName
      )
    );
    if (corrected.length === 1) {
      const correctedSubject = corrected[0];
      if (!correctedSubject) {
        return {
          status: "ambiguous"
        };
      ...
      return {
        status: "resolved",
        subjectId: correctedSubject.subjectId
      };
    }
    if (corrected.length > 1) {
      return {
...
function plannerMention(value) {
  const message = searchableMessage(value);
  return message.includes("planner") || /\bplan\b/.test(message) || message.includes("planim") || message.includes("plani");
}
async function subjectReferent(context, subjectId) {
...
async function plannerReferent(context) {
  return deepFreeze11({
    kind: "planner",
    fingerprint: await sha256({
      version: COACH_CONVERSATION_CONTEXT_V1_VERSION,
...
async function resolveCoachConversationReferentV1(input) {
  const currentSubject = explicitSubjectMatch(
    input.context,
    input.currentMessage
  );
  if (currentSubject.status === "ambiguous") {
    return deepFreeze11({
      status: "ambiguous",
      referent: null,
  ...
  if (currentSubject.status === "resolved") {
    return deepFreeze11({
      status: "resolved",
      referent: await subjectReferent(
        input.context,
  ...
  if (plannerMention(
    ...
    return deepFreeze11({
      status: "resolved",
      referent: await plannerReferent(
        input.context
      ),
      provenance: "current_message"
    });
  }
  if (!isCoachConversationFollowUpV1(
    input.currentMessage
  ) || !input.conversation) {
    return deepFreeze11({
      status: "none",
  ...
  for (let index = input.conversation.recentTurns.length - 1; index >= 0; index -= 1) {
    const turn = input.conversation.recentTurns[index];
    if (!turn) {
      continue;
    }
    if (turn.role !== "user") {
    ...
    const subject = explicitSubjectMatch(
      input.context,
      turn.text
    );
    if (subject.status === "ambiguous") {
      return deepFreeze11({
        status: "ambiguous",
        referent: null,
        provenance: `recent_user_turn:${index}`
      });
    }
...
function buildCoachConversationLanguageContextV1(input) {
  const currentMessage = normalizeText(
    input.currentMessage,
  ...
  return deepFreeze11({
    version: COACH_CONVERSATION_CONTEXT_V1_VERSION,
    authority: "non_authoritative_language_context",
    currentMessage,
...
export {
  ...
  COACH_CONVERSATION_CURRENT_MESSAGE_MAX_LENGTH_V1,
  COACH_CONVERSATION_MAX_EXCHANGES_V1,
