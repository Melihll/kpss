...
const alignToday = await api.from("tasks").update({ planned_date: smokeToday }).eq("weekly_plan_id", built.plan.id);
if (alignToday.error) throw alignToday.error;
for (const task of built.tasks) task.planned_date = smokeToday;
...
if (webDaily.dailyPlan?.totalMinutes !== today.summary.totalMinutes
  ...
  || webDaily.dailyPlan?.capacityMinutes !== today.summary.capacityMinutes) {
  throw new Error(`web and Telegram daily capacity semantics diverged: ${JSON.stringify({ web: webDaily.dailyPlan, telegram: today.summary })}`);
}
const currentPlan = await app("/weekly-plan/current");
const currentTaskById = new Map(currentPlan.tasks.map((task) => [task.id, task]));
if ((webDaily.dailyPlan?.tasks ?? []).some((task) => currentTaskById.get(task.id)?.planned_date !== webDaily.dailyPlan.date)) {
